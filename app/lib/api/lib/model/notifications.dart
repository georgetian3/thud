...
class Notifications {
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is Notifications &&
    ...
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "Notifications[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "Notifications[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return Notifications(
        notifications: mapValueOfType<Object>(json, r'notifications'),
      );
    }
    return null;
  }

  static List<Notifications> listFromJson(dynamic json, {bool growable = false,}) {
  ...
  static Map<String, List<Notifications>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<Notifications>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = Notifications.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
